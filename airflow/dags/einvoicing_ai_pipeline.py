...
SCRIPT_PATH = JOBS / "einvoicing_ai_job.py"
# The prompt, the checks and the two tables are shared with the establishments
...
with DAG(
    dag_id="einvoicing_ai",
    default_args=default_args,
    description="Writes the gold indicators up as verified sentences, with a local model",
    schedule=GOLD_ASSETS,
    catchup=False,
    # Two runs writing the same Iceberg tables corrupt each other, and the chain
    # has nothing to gain from overlapping.
    max_active_runs=1,
    tags=["einvoicing", "ai", "ollama", "iceberg", "polaris", "spark"],
) as dag:
    PythonOperator(
